  ...
  return (
    <footer id="footer" className="bg-black text-white py-10">
      <div className="max-w-6xl mx-auto px-4">
        <div className="flex flex-col md:flex-row md:justify-between md:items-start">
          ...
          >
            ...
            <div className="flex justify-center space-x-6 mb-6">
              {socialLinks.map(({ icon: Icon, href, color, ariaLabel }, index) => (
                <motion.a
                  key={index}
                  href={href}
                  target="_blank"
            ...
            >
              Let's connect — <a href={contactInfo.emailLink} className="text-blue-400 hover:text-blue-300">{contactInfo.email}</a>
            </motion.p>
          </motion.div>
          
          {/* Collaboration Section - Right Side */}
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5, delay: 0.2 }}
        ...
        {/* Copyright */}
        <motion.div 
          className="mt-8 pt-4 border-t border-neutral-900 text-center"
          initial={{ opacity: 0 }}
          whileInView={{ opacity: 1 }}
